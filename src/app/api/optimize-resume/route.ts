...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!resume.trim()) {
            console.log('No resume provided, generating from scratch based on job description and skills');
            resume = `Please create a professional resume based on the following skills: ${skills || 'general professional skills'}`;
        }

        // Generate optimized resume using LangChain and Groq
        console.log('Generating optimized resume for job description length:', jobDescription.length);
